...
/** The one search() query behind every catalog view. */
@DataJpaTest
class AssetRepositoryTest {
  ...
  void allLaptopsForAClient() {
    List<Asset> result = repository.search(1L, AssetType.LAPTOP, null, null, null);
    assertThat(result).extracting(Asset::getSerialNumber).containsExactlyInAnyOrder("SN-A", "SN-B");
  }

  @Test
  void whatIsOnDeskThree() {
    List<Asset> result = repository.search(1L, null, null, HolderType.LOCATION, 3L);
    assertThat(result).extracting(Asset::getSerialNumber).containsExactly("SN-C");
  }

  @Test
  void inStockOnly() {
    List<Asset> result = repository.search(1L, null, AssetStatus.IN_STOCK, null, null);
